...
def new_village(username: str = "Warrior") -> str:
    # Generate USERID
    ...
    village["playerInfo"]["pic"] = random.choice(["/static/socialwars/images/en/soldado_regalos.png", "/static/socialwars/images/en/soldado_regalos_w.png"])
...
def fb_friends_str(USERID: str) -> list:
    ...
    for friend_id in player_save["friends"]:
        if friend_id in __saves:
            vill = __saves[friend_id]
            frie = {
...
def neighbors(USERID: str):
    ...
    for key in __saves:
        ...
        neigh["oil"] = vill["maps"][0]["oil"]
        neigh["steel"] = vill["maps"][0]["steel"]
        # Fix profile picture paths for Flash game
        if "pic" in neigh:
            pic = neigh["pic"]
            if pic.startswith("/static/socialwars/"):
                neigh["pic"] = pic.replace("/static/socialwars/", "")
            neighbors += [neigh]
    return neighbors
