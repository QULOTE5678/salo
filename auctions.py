...
class AuctionHouse():
    def __init__(self):
        ...
        if os.path.exists(self.FILE_AH_STATE):
...
## FOR SERVER
    ...
    def set_bet(self, user_id: str, uuid: str, bet_amount: int, bet_round: int):
        if uuid in self.auctions:
            ...
            # Update timestamp to force refresh for all users
            auction["lastUpdate"] = timestamp_now()
            ...
            # Write state immediately to sync all users
            self._write_state()
            return True
        return False
    ...
    def create_auction(self, user_id: str, unit_id: int, price: int, level: int = 1):
        uuid = str(len(self.auctions) + 1)
        auction = {
            "uuid": uuid,
            "unit": unit_id,
            "level": level,
            ...
            "price": price,
            "priceIncrement": int(price * 0.1),  # 10% increment
            "betPrice": 2,
            "seller_id": user_id
        }
        
        self.config["auctions"].append(auction)
        self._write_config()
        self.update_auction(auction, timestamp_now())
        return True

    def _write_config(self):
        try:
            with open(self.FILE_AH_CONFIG, 'w') as f:
                json.dump(self.config, f, indent="\t")
        except:
            print("Error: Could not write Auction House config to disk!")
    ...
    def get_auction_detail(self, user_id: str, uuid: str, checkFinish: int):
        if uuid in self.auctions:
            ...
            # Add seller info
            bet["seller_id"] = auction_data.get("seller_id", "")
